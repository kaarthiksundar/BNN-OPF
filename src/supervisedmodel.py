...
def supervised_testing_model(opf_data: OPFData, vi_parameters = None):
    ...
    std = params['likelihood_var_prior_std']
    likelihood_std = OrderedDict([
        (name, numpyro.sample(f'l_std_{name}', normal(mean, std))) for name in params['output_block_dim'].keys()
    ])
    
    with numpyro.plate('data', size=num_data_points):
        for name in params['output_block_dim'].keys(): 
            numpyro.sample(f'Y_{name}', normal(z[name], likelihood_std[name]).to_event(1), obs=None)
...
def supervised_guide(opf_data: OPFData, vi_parameters = None):
    ...
    def create_guide_block(block_name: str):
        ...
        for i in range(num_layers):
            ...
            w_std = numpyro.param(w_std_name, w_std_init, constraint = positive)
            b_mean = numpyro.param(b_mean_name, b_mean_init)
            b_std = numpyro.param(b_std_name, b_std_init, constraint = positive)
            # create sample
        ...
        w_out_std = numpyro.param(w_out_std_name, w_out_std_init, constraint = positive)
        # create sample 
    ...
    for name in params['output_block_dim'].keys(): 
        ...
        likelihood_std_mean = numpyro.param(mean_key, vi_params.get(mean_key, mean), constraint = positive)
        likelihood_std_std = numpyro.param(std_key, vi_params.get(std_key, mean), constraint = positive)
