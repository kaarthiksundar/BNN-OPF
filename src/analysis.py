...
def run_test_pp(opf_data: OPFData, rng_key, vi_parameters, log):
    ...
    results['Eq_feasibility_all'] = Eq_feasibility_all
    results['Ineq_feasibility_all'] = InEq_feasibility_all
...
def compute_and_store_results(R):
    ...
    Eq_gap_norm_mean = (R['Eq_feasibility_mean']) # Just need to get max and mean without the norm 
    Eq_gap_mean_max = np.mean(Eq_gap_norm_mean.max(axis=1)) # Average violation over instances of the maximum over constraints
    Eq_gap_mean_mean = np.mean(Eq_gap_norm_mean) # Avearge violation over instances and over constraints
    ...
    Eq_gap_Best_Cost = Eq_gap_max_all[row_indices, Best_Cost_index]
    Eq_gap_Best_Lagrangian = Eq_gap_max_all[row_indices, Best_Lagrangian_index]
    Eq_gap_Best_Eq_gap = Eq_gap_max_all[row_indices, Best_Eq_gap_index]
    Eq_gap_Best_Ineq_gap = Eq_gap_max_all[row_indices, Best_Ineq_gap_index]

    # Compute Gaps for the Best Indices (Ineq_gap)
    Ineq_gap_Best_Cost = Ineq_gap_max_all[row_indices, Best_Cost_index]
    Ineq_gap_Best_Lagrangian = Ineq_gap_max_all[row_indices, Best_Lagrangian_index]
    Ineq_gap_Best_Eq_gap = Ineq_gap_max_all[row_indices, Best_Eq_gap_index]
    Ineq_gap_Best_Ineq_gap = Ineq_gap_max_all[row_indices, Best_Ineq_gap_index]

    # Calculate mean -- max of Eq_gap for the best indices-- # These is _max because max violation is taken first 
    # Mmaximum over constraints, minimum over weights and Average violation over instances of the
    Eq_gap_Best_Cost_max = np.mean(Eq_gap_Best_Cost) 
    Eq_gap_Best_Lagrangian_max = np.mean(Eq_gap_Best_Lagrangian)
    Eq_gap_Best_Eq_gap_max = np.mean(Eq_gap_Best_Eq_gap)
    Eq_gap_Best_Ineq_gap_max = np.mean(Eq_gap_Best_Ineq_gap)
    
    # Calculate mean and max of Ineq_gap for the best indices
    Ineq_gap_Best_Cost_max = np.mean(Ineq_gap_Best_Cost)
    Ineq_gap_Best_Lagrangian_max = np.mean(Ineq_gap_Best_Lagrangian)
    Ineq_gap_Best_Eq_gap_max = np.mean(Ineq_gap_Best_Eq_gap)
    Ineq_gap_Best_Ineq_gap_max = np.mean(Ineq_gap_Best_Ineq_gap)
    ...
    results = {
        ...
        'Cost (Best Ineq Gap)': Cost_Best_Ineq_gap,
        'Eq Gap (Best Cost)': Eq_gap_Best_Cost,
        'Eq Gap (Best Lagrangian)': Eq_gap_Best_Lagrangian,
        'Eq Gap (Best Eq Gap)': Eq_gap_Best_Eq_gap,
        'Eq Gap (Best Ineq Gap)': Eq_gap_Best_Ineq_gap,
        'Ineq Gap (Best Cost)': Ineq_gap_Best_Cost,
        'Ineq Gap (Best Lagrangian)': Ineq_gap_Best_Lagrangian,
        'Ineq Gap (Best Eq Gap)': Ineq_gap_Best_Eq_gap,
        'Ineq Gap (Best Ineq Gap)': Ineq_gap_Best_Ineq_gap,
        # 'Eq Gap Mean (Best Cost)': Eq_gap_Best_Cost_mean,
        'Eq Gap Max (Best Cost) ': Eq_gap_Best_Cost_max,
        ...
        # 'Eq Gap Mean (Best Ineq Gap)': Eq_gap_Best_Ineq_gap_mean,
        'Eq Gap Max (Best Ineq Gap)': Eq_gap_Best_Ineq_gap_max,
        # 'Ineq Gap Mean (Best Cost)': Ineq_gap_Best_Cost_mean,
        'Ineq Gap Max (Best Lagrangian)': Ineq_gap_Best_Cost_max,
        ...
        # 'Ineq Gap Mean (Best Ineq Gap)': Ineq_gap_Best_Ineq_gap_mean,
        'Ineq Gap Max (Best Ineq Gap)': Ineq_gap_Best_Ineq_gap_max
    }
...
def compute_expected_error_Bernstein_MPV(R,opf_data, delta=0.05):
    ...
    '''As Per the empirical Bernstein paper Thm 4, expected error <= estimated error + epsilon with probability at least 1-delta'''
    eps_pg = np.sqrt((2 * mean_Pred_Var_pg * np.log(1 / delta)) / N_x) + (2*2 * np.log(1 / delta)) / (3 * (N_x))
    eps_qg = np.sqrt((2 * mean_Pred_Var_qg * np.log(1 / delta)) / N_x) + (2*2 * np.log(1 / delta)) / (3 * (N_x))
    eps_vm = np.sqrt((2 * mean_Pred_Var_vm * np.log(1 / delta)) / N_x) + (2 * np.log(1 / delta)) / (3 * (N_x))
    eps_va = np.sqrt((2 * mean_Pred_Var_va * np.log(1 / delta)) / N_x) + (2 * np.log(1 / delta)) / (3 * (N_x))
...
def main():
    ...
    for file in files:
        ...
        # file = './output/pglib_opf_case118_ieee_1_1024_semisupervisedBNN_config_1.pkl'
        ...
        output_file = f'./data/{base_filename}_results.pkl'
