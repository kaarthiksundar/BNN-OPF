...
@app.command()
def train_sandwich(
        data_path = '../random_nonconvex_dataset_var70_ineq50_eq20_ex5000.npz',
    config_file: Annotated[str, typer.Option('--config', '-o')] = 'configs/default.json',
...
# ────────────────────────────── LOAD DATA ───────────────────────────────
    ...
    #shuff = numpy.random.permutation(len(X))
    #X = X[shuff]
    #Y = Y[shuff]
    N_unsup = 4096
    N_sup = 512
   ...
   #-------------------------------------- CONFIGURATION --------------------
    ...
    problem_unsup.init_meta(hidden_width=width, num_hidden_layers=num_layers)
    problem = ProblemData(
        Q=jnp.array(Q), A=jnp.array(A), G=jnp.array(G), p=jnp.array(p), h=jnp.array(h),
    ...
    for round in range(sandwich_rounds): 
        ...
        if time.time() - start_time > max_training_time:
            ...
            break
    result = {"loss1": loss1.item(), "loss2": loss2.item()}
